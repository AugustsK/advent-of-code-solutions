...
while (unresolvedRules.size) {
  ...
  unresolvedRules.forEach((rule, ruleId) => {
    ...
    const newRule = rule
      .map((ruleVariant) => {
        return ruleVariant.split(' ').reduce(
          (accumulated, ruleOrChar) => {
            ...
            return accumulated
              .map((item) => returnArr.map((str) => [item, str].filter((item) => !!item).join(' ')))
              .flat();
          },
          [null],
        );
...
resolvedRules.forEach((rule, ruleId) => {
  resolvedRules.set(
    ruleId,
    rule.map((ruleEntry) => ruleEntry.replace(/\s/g, '')),
  );
});
