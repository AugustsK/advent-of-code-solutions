...
class Line {
    ...
    getPoints() {
        ...
        } else if (this.x1 === smallestX && this.y1 === smallestY) {    // right-bottom diagonal
            let tempY = smallestY;

            for (let tempX = smallestX; tempX <= largestX; tempX++) {
                points.push(`${tempX}:${tempY++}`);
            }
        } else if (this.x2 === smallestX && this.y1 === smallestY) {    // left-bottom diagonal
            let tempY = smallestY;

            for (let tempX = largestX; tempX >= smallestX; tempX--) {
                points.push(`${tempX}:${tempY++}`);
            }
        } else if (this.x2 === smallestX && this.y2 === smallestY) {    // left-top diagonal
            let tempY = largestY;

            for (let tempX = largestX; tempX >= smallestX; tempX--) {
                points.push(`${tempX}:${tempY--}`);
            }
        } else if (this.x1 === smallestX && this.y2 === smallestY) {    // right-top diagonal
            let tempY = largestY;
